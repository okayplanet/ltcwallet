...
func (w *Wallet) RelayFee() ltcutil.Amount {
	w.relayFeeMu.Lock()
	relayFee := w.relayFee
	w.relayFeeMu.Unlock()
	return relayFee
...
// transaction) used when constructing transactions.
func (w *Wallet) SetRelayFee(relayFee ltcutil.Amount) {
	w.relayFeeMu.Lock()
	w.relayFee = relayFee
	w.relayFeeMu.Unlock()
}
...
func (w *Wallet) CurrentAddress(account uint32) (ltcutil.Address, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		scSwitch:
			...
			case txscript.PubKeyTy:
				spendable = true
			case txscript.WitnessV0PubKeyHashTy: //Todo original value WitnessPubKeyHashTy
				spendable = true
...
// wallet to disk.
func (w *Wallet) ImportPrivateKey(wif *ltcutil.WIF, bs *waddrmgr.BlockStamp,
...
func (w *Wallet) NewAddress(account uint32, addrType waddrmgr.AddressType) (addr ltcutil.Address, err error) {
	err = walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		addrmgrNs := tx.ReadWriteBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) newAddress(addrmgrNs walletdb.ReadWriteBucket, account uint32,
	addrType waddrmgr.AddressType) (ltcutil.Address, error) {
	...
	utilAddrs := make([]ltcutil.Address, len(addrs))
...
func (w *Wallet) NewChangeAddress(account uint32, addrType waddrmgr.AddressType) (addr ltcutil.Address, err error) {
	err = walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		addrmgrNs := tx.ReadWriteBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) newChangeAddress(addrmgrNs walletdb.ReadWriteBucket,
	account uint32, addrType waddrmgr.AddressType) (ltcutil.Address, error) {
